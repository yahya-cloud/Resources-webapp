...
import { showLoader, showMessage } from '../store/actions/modal'
...
const ContextProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    socket.on('callUser', ({ signal, socketId, userName }) => {
      setCall({ isReceivingCall: true, socketId, signal, userName })
    })
  ...
  const callUser = (receiverId, userName) => {
    ...
    socket.on('notFound', () => {
      dispatch(showMessage('Other user is not online'))
      setTimeout(() => {
        window.location.reload()
      }, 2000)
    })
